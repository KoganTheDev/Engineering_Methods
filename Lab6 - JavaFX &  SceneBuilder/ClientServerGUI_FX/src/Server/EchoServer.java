...
public class EchoServer extends AbstractServer 
{
 ...
 public static Student [] students=new Student[4];
  ...
  {
//	 ArrayList<String> splitMessage;
	 // Parse the input
	 String splitMessage = msg.toString();
	 
	 String[] str=splitMessage.split(" ");
	 ...
	 int flag=0;
	    ...
	    if (splitMessage.startsWith("save")) {
	    	
	    	for (int i=0;i<4;i++) {
	    		if(str.length>1  && students[i].getId().equals(str[1]))
				{
	    			
	    			
	    			students[i].setPName(str[2]);
	    			students[i].setLName(str[3]);
	    			students[i].setFc(new Faculty(str[4], ""));
	    			System.out.println("after change: "+students[i]);
				}
	    	}
	    }
